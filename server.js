...
function publicUrl(req, file) {
  return `${req.protocol}://${req.get("host")}/renders/${file}`;
}
...
function runFfmpeg(args, timeout = 420000) {
  return new Promise((resolve, reject) => {
    execFile("ffmpeg", args, { timeout }, (error, stdout, stderr) => {
      if (error) {
        reject(new Error(stderr ? String(stderr).slice(-2000) : error.message));
      } else {
...
async function renderVideoFromImages({ title, scenes, imagePaths, audioPath, durationSeconds = 180 }) {
...
app.post("/api/generate-images", async (req, res) => {
  try {
    ...
    for (let i = 0; i < Math.min(scenes.length, 8); i++) {
      const prompt = scenes[i].visualPrompt || scenes[i].text || `cute children's story scene ${i + 1}`;
      const img = await generateImageFile(prompt, i);
      images.push({ file: `/renders/${img.fileName}`, url: publicUrl(req, img.fileName) });
    }

    res.json({ ok: true, images });
  } catch (err) {
    res.status(500).json({ ok: false, error: "generate-images failed", details: err.message });
  }
});
...
app.post("/api/generate-voice", async (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      file: `/renders/${voice.audioName}`,
      url: publicUrl(req, voice.audioName)
    });
  } catch (err) {
...
app.post("/api/create-full-video", async (req, res) => {
  try {
    ...
    for (let i = 0; i < scenes.length; i++) {
      const prompt = scenes[i].visualPrompt || scenes[i].text || topic;
      images.push(await generateImageFile(prompt, i));
    }
    ...
    const video = await renderVideoFromImages({
      title: story.title || "TinyMind Kids Video",
      scenes,
